...
class ZI_Util
{
	...
	GameRoundWin = SpawnEntityFromTable("game_round_win", {targetname = "__utilroundwin", TeamNum = TF_TEAM_BLUE, force_map_reset = 1})
	...
	function ChangePlayerTeamMvM(player, teamnum = TF_TEAM_BLUE)
	{
		if (this.GameRules) {
	...
	function CountAliveBots(printout = false)
	{
		...
		foreach (bot in this.ZombieArray)
		{
			if (this.IsAlive(bot))
				botsalive++
		}
	...
	{
		this.ClientCommand.AcceptInput("Command", format("slot%d", slot + 1) player, player)
	}
	...
	function Explanation(message, printColor = COLOR_YELLOW, messagePrefix = "Explanation: ", syncChatWithGameText = false, textPrintTime = -1, textScanTime = 0.02) {
		DoExplanation.call(ZI_Util, message, printColor, messagePrefix, syncChatWithGameText, textPrintTime, textScanTime)
	}
	...
	function StunPlayerOld(player, duration = 5, type = 1, delay = 0, speedreduce = 0.5) {
	...
	function AddThinkToThinkTable(ent, func)
	{
		local scope = ent.GetScriptScope()
		local thinktable = "ThinkTable"
	...
	function SilentDisguise(player, target = null, tfteam = TF_TEAM_BLUE, tfclass = TF_CLASS_SCOUT) {
	...
	function OnWeaponFire(wep, func) {
		...
		scope.ThinkTable[format("OnWeaponFire_%d_%d", wep.GetOwner().GetScriptScope().userid, wep.entindex())] <- function() {
	...
	Events = {
        ...
        // leftover from mvm, bots are allocated before this event
		...
		// 		if (player != null && player.GetTeam() == TF_TEAM_BLUE && ZI_Util.ZombieArray.find(player) == null)
		// 			ZI_Util.ZombieArray.append(player)

		// 		else if (player != null && !player.GetTeam() == TF_TEAM_BLUE && ZI_Util.HumanArray.find(player) == null)
		// 			ZI_Util.HumanArray.append(player)

		// 		if (player != null && ZI_Util.PlayerArray.find(player) == null)
		// 			ZI_Util.PlayerArray.append(player)
		// 	}
		// }
		...
		// 		{
		// 			scope.Thinks <- function() { foreach (name, func in scope.ThinkTable) func.call(scope); return -1 }
		// 			AddThinkToEnt(child, "Thinks")
		// 		}
		// 	}

		// 	if (player.GetTeam() == TF_TEAM_BLUE && ZI_Util.ZombieArray.find(player) == null)
		// 		ZI_Util.ZombieArray.append(player)

		// 	else if (!player.GetTeam() == TF_TEAM_BLUE && ZI_Util.HumanArray.find(player) == null)
		// 		ZI_Util.HumanArray.append(player)

		// 	if (ZI_Util.PlayerArray.find(player) == null)
		// 		ZI_Util.PlayerArray.append(player)
		// }
		...
		// 	if (!player.GetTeam() == TF_TEAM_BLUE && ZI_Util.HumanArray.find(player) == null)
		// 		ZI_Util.HumanArray.append(player)

		// 	else if (ZI_Util.PlayerArray.find(player) == null)
		// 		ZI_Util.PlayerArray.append(player)
		// }
		...
		// 	for (local i = ZI_Util.HumanArray.len() - 1; i >= 0; i--)
		// 		if (ZI_Util.HumanArray[i] == null || ZI_Util.HumanArray[i] == player)
		// 			ZI_Util.HumanArray.remove(i)

		// 	for (local i = ZI_Util.PlayerArray.len() - 1; i >= 0; i--)
		// 		if (ZI_Util.PlayerArray[i] == null || ZI_Util.PlayerArray[i] == player)
		// 			ZI_Util.PlayerArray.remove(i)
		// }
	}
}

ZI_Util.TriggerHurt.DispatchSpawn()

NavMesh.GetAllAreas(ZI_Util.AllNavAreas)

// __CollectGameEventCallbacks(ZI_Util.Events)

ZI_Util.RespawnOverride.SetSolid(2)
ZI_Util.RespawnOverride.SetSize(Vector(), Vector(1, 1, 1))
...
ZI_Util.RespawnOverride.ValidateScriptScope();
ZI_Util.RespawnOverride.GetScriptScope().InputStartTouch <- RespawnStartTouch;
ZI_Util.RespawnOverride.GetScriptScope().Inputstarttouch <- RespawnStartTouch;
ZI_Util.RespawnOverride.GetScriptScope().InputEndTouch <- RespawnEndTouch;
ZI_Util.RespawnOverride.GetScriptScope().Inputendtouch <- RespawnEndTouch;
